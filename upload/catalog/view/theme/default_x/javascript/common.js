...
$(document).ready(function () {
	if (localStorage.getItem('miniCart') == 'active') {
		$('.listMenu__link.bars, .listMenu__link.miniCart').addClass('listMenu__link-cartActive');
	} else {
		$('.listMenu__link.bars, .listMenu__link.miniCart').removeClass('listMenu__link-cartActive');
	}
});
	...
	'update': function (key, quantity) {
		$.ajax({
			...
			success: function (json) {
				...
				setTimeout(function () {
					var quantityCart = json['total'].replace(/[^0-9]/gim,'');
					if (quantityCart[0] == 0) {
						$('.listMenu__link.bars, .listMenu__link.miniCart').removeClass('listMenu__link-cartActive');
						localStorage.setItem('miniCart', 'disabled');
					}
				}, 100);
